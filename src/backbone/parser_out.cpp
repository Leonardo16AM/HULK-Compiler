...
parser_out::parser_out(const std::vector<grammar_production> &derivations, int error)
    : ok(error == -1), error(error), tree(ok ? build_tree(derivations) : nullptr) {}

std::shared_ptr<derivation_tree> parser_out::build_tree(const std::vector<grammar_production> &derivations) {
    auto root = std::make_shared<derivation_tree>(derivations[0].head);
    build_tree_node(root, derivations, 0);
    return root;
}

int parser_out::build_tree_node(std::shared_ptr<derivation_tree> node, const std::vector<grammar_production> &derivations, int index) {
